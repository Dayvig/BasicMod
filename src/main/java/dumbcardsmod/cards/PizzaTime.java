...
public class PizzaTime extends BaseCard {

    private final static CardInfo cardInfo = new CardInfo(
            "PizzaTime", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new MakeTempCardInDiscardAction(new Pizza(), magicNumber));
        CardCrawlGame.music.silenceBGMInstantly();
        CardCrawlGame.music.silenceTempBgmInstantly();
        AbstractDungeon.getCurrRoom().playBGM("Pizza");
    }

    @Override
    public AbstractCard makeCopy() { //Optional
        return new PizzaTime();
    }
}
