...
public class PlayTheUnplayable extends BaseCard {

    private final static CardInfo cardInfo = new CardInfo(
            "PlayTheUnplayable", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new PlayTheUnplayablePower(p, magicNumber), magicNumber));
    }

    @Override
    public AbstractCard makeCopy() { //Optional
        return new PlayTheUnplayable();
    }
}
