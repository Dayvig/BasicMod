...
import dumbcardsmod.powers.HeadEmptyPower;
import dumbcardsmod.util.CardInfo;

import static dumbcardsmod.DumbCardsMod.makeID;

public class NoThoughts extends BaseCard {

    private final static CardInfo cardInfo = new CardInfo(
            "NoThoughts", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new HeadEmptyPower(p, magicNumber), magicNumber));
    }


    @Override
    public AbstractCard makeCopy() { //Optional
        return new NoThoughts();
    }
}
